...
# based on StarGAN-VC discriminator
...
# this is from Fede, Lukas and Shuai
class TDNN(nn.Module):
    def __init__(self, feat_dim=40, num_speakers=7146, emb_size=512, hidden=256):
        super(TDNN, self).__init__()
        self.hidden_dim = hidden  # old 1024
        self.pre_pool_out = int(1000*np.ceil(self.hidden_dim/1000))
        self.pre_pool = nn.Sequential(
            nn.Conv1d(feat_dim, self.hidden_dim, 5),
            nn.ReLU(),
            nn.BatchNorm1d(self.hidden_dim, affine=False),
            nn.Conv1d(self.hidden_dim, self.hidden_dim, 5, dilation=2),
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.BatchNorm1d(self.hidden_dim, affine=False),
            ...
            nn.Dropout(p=0.2),
            nn.BatchNorm1d(self.hidden_dim, affine=False),
            nn.Conv1d(self.hidden_dim, self.hidden_dim, 3, dilation=3),
            nn.ReLU(),
            nn.Dropout(p=0.2),
            nn.BatchNorm1d(self.hidden_dim, affine=False),
            nn.Conv1d(self.hidden_dim, self.pre_pool_out, 1),
            nn.ReLU(),
            nn.BatchNorm1d(self.pre_pool_out, affine=False)
        )
        
        self.dense_6 = nn.Linear(2*self.pre_pool_out, emb_size)
        self.bn_6 = nn.BatchNorm1d(emb_size, affine=False)
        self.dense_7 = nn.Linear(emb_size, emb_size)
        self.bn_7 = nn.BatchNorm1d(emb_size, affine=False)
        self.dense_8 = nn.Linear(emb_size, num_speakers)
    def forward(self, x):
        x = torch.cat([x[:,:,:1].repeat(1,1,9), x, x[:,:,-1:].repeat(1,1,9)], 2)
        out = self.pre_pool(x)
        pooling_mean = torch.mean(out, dim=2)
        pooling_std = torch.std(out, dim=2, unbiased=True)
        stats = torch.cat((pooling_mean, pooling_std), 1)
        embedding_1 = self.dense_6(stats)
        out = F.relu(embedding_1)
        out = self.bn_6(out)

        embedding_2 = self.dense_7(out)
...
# AutoVC model is from original repository
...
class Generator(nn.Module):
    ...
    def __init__(self, dim_neck=16, dim_emb=256, dim_pre=512, freq=16):
    ...
    def forward(self, x, c_org, c_trg):
        ...
        for code in codes:
            t = code.unsqueeze(1)
            tmp.append(t.expand(-1, int(x.size(1) / len(codes)), -1))
            tmp_out.append(t)

        code_exp = torch.cat(tmp, dim=1)
        code_out = torch.cat(tmp_out, dim=1)
        encoder_outputs = torch.cat((code_exp, c_trg.unsqueeze(1).expand(-1, x.size(1), -1)), dim=-1)

        mel_outputs = self.decoder(encoder_outputs).transpose(2, 1)

        mel_outputs_postnet = self.postnet(mel_outputs)
        mel_outputs_postnet = mel_outputs + mel_outputs_postnet

        return mel_outputs, mel_outputs_postnet, torch.cat(codes, dim=-1), code_out
